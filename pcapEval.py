import dpkt, socket, dpkt.rtp, os, pylab, csv, time
from matplotlib.backends.backend_pdf import PdfPages

'''
# TODO Setup to check for packages needed, and do installs if they are not in the Python cofig
install dpkt
install numpy
install scipy
having trouble with lapack/blas
install pylab
# TODO add similar thing to PathView
'''
...
'''
# TODO Add frame rate array, and include in graphs
#  Mark shows last packet of a frame, next packet is first packet.
payloadTypes = [
    (0, 'Audio', 'PCMU', 8000),
    (1, 'Audio', 'Reserved',),
    (2, 'Audio', 'G721',),
...
class Stream():
    def __init__(self, streamTup, encap):
    ...
    def parsePkt(self, data):
        if self.encap == 'ethernet':
            link = dpkt.ethernet.Ethernet(data)
        elif self.encap == 'linux-cooked':
            link = dpkt.sll.SLL(data)
        ip = link.data
        udp = ip.data
        rtp = dpkt.rtp.RTP(udp.data)
        return (link, ip, udp, rtp)
    ...
    def rtpTsDelta(self):
        ...
        ether, ip, udp, rtp = self.parsePkt(data)
        rtp_tsl = rtp.ts
    ...
    def clockEst(self):
        return (self.lastRtpTs - self.firstRtpTs) / len(self)

    def addPkt(self, ts, pkt):
        self.pkts.append((ts, pkt))
        self.len += 1
        if self.ptype == None:
            ether, ip, udp, rtp = self.parsePkt(pkt)
    ...
    def findBW(self):
        ...
        for pkt in self.pkts:
            ...
            ip = ether.data
            bytes += len(ip) + 18 #ip len plus ethernet header & trailer
    ...
    def findBWArray(self, slot):
        ...
        for pkt in self.pkts:
            ...
            if tsRel > (slotNow + 1) * slot:
                ...
                slotNow += 1
            bytes += len(ip) + 18 #ip len plus ethernet header & trailer
            totBytes += len(ip) + 18 #ip len plus ethernet header & trailer
    ...
    def find_frame_rate(self):
        ...
        for pkt in self.pkts:
            ...
            if rtp.m:
                frames += 1
        self.frame_rate = float(frames) / self.tsDelta()
        return self.frame_rate
...
def splitPcapRTP(pcapFile, types):
    ...
    fmt = '{0:1s} {1:12s} {2}'
    print(fmt.format(' ', 'FirstPktNum', 'Stream Name'))
    for ts, data in pcapFile:
        pktNum += 1
        if pcapFile.dloff == 14:
            encap = 'ethernet'
            frame = dpkt.ethernet.Ethernet(data)
            if frame.type != dpkt.ethernet.ETH_TYPE_IP:
                continue
        elif pcapFile.dloff == 16:
            encap = 'linux-cooked'
            frame = dpkt.sll.SLL(data)
            if frame.ethtype != dpkt.ethernet.ETH_TYPE_IP:
                continue
        else:
            raise ValueError('Unimplemented link type')
        ...
        try:
            ...
            print('Packet length ', len(udp), len(ip), len(frame), len(data))
            raise
        if rtp.version != 2: continue
        if types[rtp.pt][2] == 'Unassigned': continue
        if types[rtp.pt][2] == 'Reserved': continue
        if types[rtp.pt][1] == 'N/A': continue
        thisTup = (ip.src, udp.sport, ip.dst, udp.dport, rtp.ssrc)
        try:
            stream = streams[thisTup]
        except:
            streams[thisTup] = Stream(thisTup, encap)
            stream = streams[thisTup]
            stream.ptype = rtp.pt
            fmt = '{0:6s} {1:7d} {2:s}'
            print(fmt.format('found:', pktNum, stream))
...
def findSlot(stream):
    ...
    intArray = [(30, 0.1), (150, 0.5), (300, 1), (600, 2), (1500, 5), (3000, 10), (4500, 15), (9000, 30), (0, 60)]
...
def setUpPlots():
    ...
    plot = raw_input(
        'What plot types do you want? (bw, loss, pps, jitter, jitterloss, outoforder, all, done)? ').replace(' ',
                                                                                                             '').lower().split(
        ',')
    if plot == ['']:
        return plotTypes
...
def displayStreams(streams):
    ...
    fmt = '{0:8s} {1:13s} {2:s}'
    print(fmt.format('Packets', 'Stream Type', 'Stream Name'))
    for key in streams:
        fmt = '{0:7d} {1:16s} {2:s}'
        print(fmt.format((streams[key].len), payloadTypes[streams[key].ptype][2], printTup(key)))
        #print printTup(key)
    print('\n')
...
def overview(streams):
    fmtData = '{0[0]:>55s} {0[1]:3d} {0[2]:18s} {0[3]:6d} {0[4]:7.0f}K {0[5]:7.0f} {0[6]:0=6.2%} {0[7]:0=6.2%} {0[8]:0=6.2%} {0[9]:5s} {0[10]:5.0f} {0[11]:5.1f} {0[12]:14s} {0[13]:4d} {0[14]:5.1f}'
    fmtHdr = '{0[0]:^55s} {0[1]:3s} {0[2]:^18s} {0[3]:>6s} {0[4]:>7s} {0[5]:>7s} {0[6]:^6s} {0[7]:^6s} {0[8]:^6s} {0[9]:5s} {0[10]:5s} {0[11]:^5s} {0[12]:^23s} {0[13]:6s} {0[14]:6s}'
    hdrRow = ['ipSrc-udpSrcPort-ipDst-udpDstPort-SSRC', 'Typ', 'Codec', 'Pkts', 'BW ', 'PPS', 'Loss', 'JtLs', 'OOO',
              'Cksum', 'ClkDiv', 'Len', 'Start', 'DSCP', 'FR']
    print
    ...
    if outFileName.lower in ['y', 'yes', 'yep']:
        outFileName = raw_input('Will create CSV format file. Filename? ')
    if outFileName.lower not in ['n', 'no']:
